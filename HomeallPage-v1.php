...
$selected_year = isset($_GET['year']) ? (int)$_GET['year'] : 0; // พารามิเตอร์ใหม่สำหรับปี
...
<aside class="w-64 bg-white shadow-lg p-6 flex flex-col sticky top-0 h-screen">
    <div class="flex flex-col items-center border-b pb-4 mb-4">
        <!-- โลโก้ PSU พร้อม fallback image -->
    ...
    <!-- ปุ่มสำหรับเข้าสู่ระบบเจ้าหน้าที่ -->
...
<main class="flex-1 p-8">
    <header class="flex flex-col md:flex-row items-center justify-between mb-8 pb-4 border-b border-gray-300">
        ...
        <!-- Search Form (ใช้สำหรับค้นหาทั้งอาจารย์และงานตีพิมพ์) -->
    ...
    <!-- รายชื่ออาจารย์ (ถูกกรองตาม Search Query) -->
    <section class="bg-white p-6 rounded-xl shadow-xl mb-8">
        ...
        if (!empty($search_query)) {
            // กรองด้วยชื่ออาจารย์: (first_name LIKE %query% OR last_name LIKE %query%)
    ...
    <!-- งานตีพิมพ์ (กรองตามปี และ ค้นหาตามชื่อเรื่อง/ชื่ออาจารย์) -->
    <section class="bg-white p-6 rounded-xl shadow-xl">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b pb-2">
            ...
            <form method="GET" action="HomeallPage-v1.php" class="flex items-center space-x-2 w-full sm:w-auto">
                <!-- เก็บค่า Search Query ไว้ ถ้ามี -->
        ...
        if (!empty($search_query)) {
            // ค้นหาจากชื่อเรื่อง (p.title) หรือ ชื่อผู้จัดทำ (CONCAT(u.first_name,' ',u.last_name))
